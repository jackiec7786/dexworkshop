...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const schedDate = b.scheduled_date !== undefined
    ? (b.scheduled_date || null)   // "" or null → null (unschedule)
    : undefined;
  ...
  const rows = await sql`
    UPDATE jobs SET
      ...
      scheduled_date = CASE WHEN ${schedDate !== undefined}::boolean
                         THEN ${schedDate ?? null}::date
